  ...
  >
    <a-form :form="form">
      ...
      <a-form-item
        ...
        label="新密码"
      >
        <a-input type="password" placeholder="新密码" v-decorator="['newpassword', {rules: [{ required: true,min:6,max:16, message: '请输入6到16位密码' }]}]" />
      </a-form-item>
...
export default {
  ...
  methods: {
    edit (record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.mdl, 'userId', 'userName'))
      })
    },
    handleSubmit (e) {
      e.preventDefault()
